...
internal class CoverPlayerView(
    ...
    // View.onTouchEvent dispatches performClick for accepted background taps, including accessibility.
    @SuppressLint("ClickableViewAccessibility")
    override fun onTouchEvent(event: MotionEvent): Boolean {
        if (event.actionMasked == MotionEvent.ACTION_DOWN) {
            // Disabled controls must not behave as a tap on the background.
            val occupied = functionalViews
                .any { it.isVisible && event.x >= it.left && event.x <= it.right && event.y >= it.top && event.y <= it.bottom }
